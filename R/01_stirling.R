...
# Keep one session-local table: a table computed to J_max contains every
# smaller table.  The cache is deliberately bounded to one matrix so repeated
# PMF calls do not repeatedly allocate O(J^2) memory or grow memory without a
# deterministic limit.
.dpprior_stirling_cache <- local({
...
#' Inspect the Session-local Stirling Cache
#'
#' Internal diagnostic helper used to audit cache provenance and memory.
#'
#' @return A list containing the cache policy, largest cached J, hit/miss
#'   counts, and retained bytes.
#'
#' @keywords internal
.stirling_cache_info <- function() {
  list(
    policy = "single-largest-table",
    max_J = .dpprior_stirling_cache$max_J,
    hits = .dpprior_stirling_cache$hits,
    misses = .dpprior_stirling_cache$misses,
    entries = if (is.null(.dpprior_stirling_cache$logS)) 0L else 1L,
    bytes = if (is.null(.dpprior_stirling_cache$logS)) {
      0
    } else {
      as.numeric(utils::object.size(.dpprior_stirling_cache$logS))
    }
  )
}


#' Clear the Session-local Stirling Cache
#'
#' Internal testing and benchmarking helper.
#'
#' @return Invisibly returns \code{TRUE}.
#'
#' @keywords internal
.clear_stirling_cache <- function() {
  .dpprior_stirling_cache$logS <- NULL
  .dpprior_stirling_cache$max_J <- -1L
  .dpprior_stirling_cache$hits <- 0L
  .dpprior_stirling_cache$misses <- 0L
  invisible(TRUE)
}
...
compute_log_stirling <- function(J_max) {
  ...
  if (!is.numeric(J_max) || length(J_max) != 1L || !is.finite(J_max) ||
    ...
    .dpprior_abort_legacy_numeric(
      "J_max must be a non-negative integer", J_max, "J_max",
      "non-negative integer", "dpprior_count_error",
      scalar = TRUE, integer = TRUE
    )
  }
  if (J_max > .MAX_J_DEFAULT) {
    .dpprior_abort_invalid(
      sprintf("J_max exceeds maximum supported value (%d)", .MAX_J_DEFAULT),
      c("dpprior_count_error", "dpprior_bounds_error"), "J_max", J_max,
  ...
  # A cached table for a larger J contains the requested leading submatrix.
  # Subsetting returns an ordinary matrix and preserves the public API.
  if (!is.null(.dpprior_stirling_cache$logS) &&
      .dpprior_stirling_cache$max_J >= J_max) {
    .dpprior_stirling_cache$hits <- .dpprior_stirling_cache$hits + 1L
    idx <- seq_len(J_max + 1L)
    return(.dpprior_stirling_cache$logS[idx, idx, drop = FALSE])
  }

  .dpprior_stirling_cache$misses <- .dpprior_stirling_cache$misses + 1L
  ...
  # Handle J_max = 0 case
  if (J_max == 0L) {
    .dpprior_stirling_cache$logS <- logS
    .dpprior_stirling_cache$max_J <- J_max
    return(logS)
  }
