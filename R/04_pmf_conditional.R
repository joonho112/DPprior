...
# ---------------------
# To avoid numerical overflow, we compute the Antoniak weights in log-space
# and normalize them with log-sum-exp over the mathematical support 1:J.
#
# Dependencies:
# -------------
...
.check_logS_size <- function(J, logS) {
  if (!is.matrix(logS) || !is.numeric(logS)) {
    .dpprior_abort_invalid(
      "logS must be a numeric matrix returned by compute_log_stirling()",
      c("dpprior_stirling_table_error", "dpprior_type_error"),
      "logS", logS, "numeric matrix", "type"
    )
  }
  if (nrow(logS) != ncol(logS)) {
    .dpprior_abort_invalid(
      "logS must be a square matrix returned by compute_log_stirling()",
      c("dpprior_stirling_table_error", "dpprior_dimension_error"),
      "logS", logS, "square matrix", "dimension"
    )
  }
  if (J + 1L > nrow(logS)) {
    .dpprior_abort_invalid(
      sprintf(
        "Stirling matrix too small: J=%d but matrix has J_max=%d",
        J, nrow(logS) - 1L
      ),
      c("dpprior_stirling_table_error", "dpprior_bounds_error"),
      "logS", logS, sprintf("table supporting J >= %d", J), "bounds"
    )
  }

  support_row <- logS[J + 1L, seq.int(2L, J + 1L)]
  if (anyNA(support_row) || any(!is.finite(support_row))) {
    .dpprior_abort_invalid(
      sprintf("logS row for J=%d must be finite on support k=1,...,J", J),
      c("dpprior_stirling_table_error", "dpprior_nonfinite_error"),
      "logS", logS, "finite support row", "nonfinite"
    )
  }
  invisible(TRUE)
}
...
#' is exact, subtracting the two log-gamma values loses accuracy when
#' \eqn{\alpha} is large relative to \eqn{J}. The implementation instead uses
#' finite sums with \code{log1p()} corrections, choosing the algebraically
#' equivalent branch whose ratios are small.
#'
#' @examples
#' \dontrun{
#' # (2)_3 = 2 * 3 * 4 = 24
...
log_rising_factorial <- function(alpha, J) {
  ...
  alpha <- .dpprior_validate_scalar(
    ...
    .subclass = "dpprior_concentration_error"
  )
  J <- as.integer(J)
  ...
  r <- seq_len(J - 1L)
  if (alpha >= J) {
    return(J * log(alpha) + sum(log1p(r / alpha)))
  }

  log(alpha) + sum(log(r) + log1p(alpha / r))
}
...
#'   Entry \code{[1]} corresponds to \eqn{k=0} and always equals \code{-Inf}.
#'   The mathematical support is \eqn{\{1,\ldots,J\}}; the leading structural
#'   zero is retained in the vector representation for compatibility.
#'
#' @details
...
#' \eqn{(\alpha)_J} is the rising factorial. The implementation normalizes the
#' equivalent log-weights over \eqn{k=1,\ldots,J} with log-sum-exp. This avoids
#' cancellation in a separately evaluated log-gamma normalizer at very large
#' finite \eqn{\alpha}.
#'
...
log_pmf_K_given_alpha <- function(J, alpha, logS) {
  ...
  # The common Antoniak normalizer can be eliminated by normalizing the
  # finite log-weights directly. J <= .MAX_J_DEFAULT keeps k*log(alpha)
  # finite for every accepted finite double alpha.
  k <- seq_len(J)
  log_weight <- logS[J + 1L, k + 1L] + k * log_alpha
  if (any(!is.finite(log_weight))) {
    .dpprior_abort_invalid(
      "conditional log-PMF weights must be finite on support k=1,...,J",
      c("dpprior_conditional_pmf_error", "dpprior_nonfinite_error"),
      "alpha", alpha, "finite conditional log-PMF weights", "nonfinite"
    )
  }

  log_normalizer <- logsumexp_vec(log_weight)
  if (!is.finite(log_normalizer)) {
    .dpprior_abort_invalid(
      "conditional log-PMF normalizer must be finite",
      c("dpprior_conditional_pmf_error", "dpprior_nonfinite_error"),
      "alpha", alpha, "finite log normalizer", "nonfinite"
    )
  }

  c(-Inf, log_weight - log_normalizer)
}
...
#' @param logS Matrix; pre-computed log-Stirling matrix from
#'   \code{\link{compute_log_stirling}}.
#' @param normalize Logical; if \code{TRUE} (default), apply a final
#'   probability-scale normalization. If \code{FALSE}, directly exponentiate
#'   the already normalized log-PMF.
#'
#' @return Numeric vector of length \eqn{J+1} containing
#'   \eqn{P(K_J = k \mid \alpha)} for \eqn{k = 0, 1, \ldots, J}.
#'   Entry \code{[1]} corresponds to \eqn{k=0} and always equals 0. The
#'   mathematical support is \eqn{\{1,\ldots,J\}}. The vector sums to 1 up to
#'   floating-point tolerance in either normalization mode.
#'
#' @details
...
#' }
#'
#' Extremely small support probabilities may underflow to structural zeros on
#' the probability scale. Setting \code{normalize = TRUE} applies a final
#' normalization after exponentiation; \code{normalize = FALSE} skips only
#' that final correction and does not return unnormalized Antoniak weights.
#'
#' @examples
...
pmf_K_given_alpha <- function(J, alpha, logS, normalize = TRUE) {
  normalize <- .dpprior_validate_control(
    normalize, "normalize", type = "logical"
  )
  log_pmf <- log_pmf_K_given_alpha(J, alpha, logS)
  probability <- exp(log_pmf)
  probability[1L] <- 0

  if (normalize) {
    support_total <- sum(probability[-1L])
    if (!is.finite(support_total) || support_total <= 0) {
      .dpprior_abort_invalid(
        "conditional PMF must have finite positive support mass",
        c("dpprior_conditional_pmf_error", "dpprior_pmf_mass_error"),
        "alpha", alpha, "finite positive support mass", "zero_mass"
      )
    }
    probability[-1L] <- probability[-1L] / support_total
  }

  .dpprior_validate_pmf(
    probability, "conditional_pmf", expected_length = length(log_pmf)
  )
}
...
#' Edge cases:
#' \itemize{
#'   \item \eqn{p = 0}: returns 1 (the lower endpoint of the support)
...
#'
#' # Edge cases
#' quantile_K_given_alpha(0, 50, 2.0, logS)  # Returns 1
...
quantile_K_given_alpha <- function(p, J, alpha, logS) {
  p <- .dpprior_validate_probability(p, "p", scalar = FALSE)
  assert_valid_J(J)
  J <- as.integer(J)
  ...
  qfun <- function(pp) {
    ...
    # cdf[-1] is indexed by the mathematical support k=1,...,J.
    idx <- which(cdf[-1L] >= pp)[1L]
    if (is.na(idx)) {
      .dpprior_abort_invalid(
        "conditional quantile inversion failed for a valid probability",
        c("dpprior_conditional_pmf_error", "dpprior_numerical_error"),
        "p", pp, "invertible conditional CDF", "inversion"
      )
    }
    as.integer(idx)
  }
