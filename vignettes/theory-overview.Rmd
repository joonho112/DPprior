...
$$
where $\mathcal{F}_{i-1}$ is the $\sigma$-field induced by the seating 
...
# Build one bounded-discrete target and hold it fixed across methods
target_K <- DPprior_target_K(J = J, mu_K = mu_K, var_K = var_K)

# A1 and A2 solutions for the same canonical target
fit_a1 <- DPprior_fit(
  J = J, target_K = target_K, method = "A1",
...
comparison <- data.frame(
  ...
  a = c(fit_a1$parameters$a, fit_a2$parameters$a),
  b = c(fit_a1$parameters$b, fit_a2$parameters$b),
  Achieved_Mean = c(fit_a1$achieved$K$mean, fit_a2$achieved$K$mean),
  Achieved_Var = c(fit_a1$achieved$K$variance,
                   fit_a2$achieved$K$variance),
  Status = c(fit_a1$status, fit_a2$status),
  Usable = c(fit_a1$usable, fit_a2$usable),
  Verified = c(fit_a1$verified, fit_a2$verified)
)
...
a_init <- fit_a1$parameters$a
b_init <- fit_a1$parameters$b
...
for (i in 2:(n_iter + 1)) {
  ...
  t <- i - 1
  trajectory$a[i] <- fit_a2$parameters$a +
    (a_init - fit_a2$parameters$a) * (0.1)^t
  trajectory$b[i] <- fit_a2$parameters$b +
    (b_init - fit_a2$parameters$b) * (0.1)^t
...
**Novel Contribution.** Analysis in [Lee (2026, arXiv:2602.06301)](https://arxiv.org/abs/2602.06301) shows that the
...
error_data <- do.call(rbind, lapply(J_values, function(J) {
  ...
  data.frame(
    J = J,
    Mean_Error_Pct = 100 * abs(fit_a1$achieved$K$mean - mu_K) / mu_K,
    Var_Error_Pct = 100 * abs(fit_a1$achieved$K$variance - var_K) / var_K,
    a_Relative_Error = abs(fit_a1$parameters$a - fit_a2$parameters$a) /
      fit_a2$parameters$a,
    b_Relative_Error = abs(fit_a1$parameters$b - fit_a2$parameters$b) /
      fit_a2$parameters$b
  )
}))
...
| `DPprior_target_K(J, ...)` | Canonical bounded-discrete target constructor |
| `DPprior_fit(J, target_K = ...)` | Main calibration wrapper |
