...
**First stick-breaking weight distribution:**

```{r cct-viz-w1, fig.height=4, fig.cap="Prior distribution on the first stick-breaking / size-biased weight w1. The dashed line at 0.5 indicates the dominance threshold."}
...
The diagnostics reveal an important concern: **`r diag_cct$weights$dominance_risk` 
dominance risk** with $P(w_1 > 0.5) = `r round(diag_cct$weights$prob_exceeds["prob_gt_0.5"], 2)`$. 
This means there is a `r round(100 * diag_cct$weights$prob_exceeds["prob_gt_0.5"])`% 
...
probability can lead to overshrinkage in posterior inference. The dual-anchor 
framework allows us to constrain the prior to reduce dominance while 
maintaining our beliefs about the expected number of clusters.

**Applying dual-anchor refinement:**

We target $P(w_1 > 0.5) = 0.25$ (25% dominance probability) while keeping 
$K$ as the primary anchor ($\lambda = 0.7$):
...
```{r cct-comparison-viz, fig.height=6, fig.cap="Comparison of K-only and dual-anchor priors: the dual-anchor approach reduces dominance risk while maintaining similar cluster count expectations."}
...
The dual-anchor prior shifts mass in the $w_1$ distribution away from 
...
> calibration revealed high dominance risk with $P(w_1 > 0.5) = `r round(diag_cct$weights$prob_exceeds["prob_gt_0.5"], 2)`$, 
> which is problematic in settings with limited between-site information. 
> We therefore applied dual-anchor refinement targeting $P(w_1 > 0.5) \leq 0.25$ 
> with $\lambda = 0.7$, yielding a Gamma(`r round(fit_cct_dual$a, 3)`, 
> `r round(fit_cct_dual$b, 3)`) hyperprior. The refined prior maintains 
> $\mathbb{E}[K_J] \approx `r round(diag_cct_dual$K$mean, 1)`$ while reducing 
> dominance risk to `r diag_cct_dual$weights$dominance_risk`. Sensitivity 
...
6. **Diagnostic summary**: Key quantities like $P(w_1 > 0.5)$ and dominance risk
...
3. **Dual-anchor refinement**: When dominance risk is high, use 
   `DPprior_dual()` to control weight behavior
