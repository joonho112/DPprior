...
| "Moderately confident" | $\text{Var}(K_J) \approx 2.5 \times (\mu_K - 1)$ |
| "Very uncertain" | $\text{Var}(K_J) \approx 5 \times (\mu_K - 1)$ |
...
grid search with fast closed-form initialization and Newton refinement for
accurate moment matching. The optional dual-anchor extension
further incorporates weight constraints, addressing the "unintended prior" 
problem identified by Vicentini & Jermyn (2025).

The low-confidence specification below is deliberately diffuse. We therefore
use 160 quadrature nodes for that fit; DPprior independently verifies its
diagnostics at the required higher order rather than accepting an approximate
diagnostic bundle.
...
# Method 1: Using confidence levels. The diffuse low-confidence prior uses
# a higher quadrature order so its diagnostics pass independent verification.
fit_low <- DPprior_fit(J = J, mu_K = mu_K, confidence = "low", M = 160L)
...
results <- data.frame(
  ...
  E_alpha = round(c(fit_low$parameters$a / fit_low$parameters$b,
                    fit_med$parameters$a / fit_med$parameters$b,
                    fit_high$parameters$a / fit_high$parameters$b), 3)
)
...
cat("Elicited prior: α ~ Gamma(", round(fit$parameters$a, 3), ", ",
    round(fit$parameters$b, 3), ")\n", sep = "")
...
# Check the named first-size-biased-weight estimand
p_wsb <- prob_wsb_exceeds(
  0.5, fit_K$parameters$a, fit_K$parameters$b
)
cat("K-only prior: P(W_SB > 0.5) =", round(p_wsb, 3), "\n")

# A hard scientific requirement is an independently verified inequality.
...
fit_hard[c("status", "usable", "verified")]
fit_hard$constraint[c("satisfied", "residual")]

# A soft question instead declares a fixed-scale trade-off.
...
fit_soft[c("status", "usable", "verified")]
```
...
| **B. Elicitation Engine** | Mapping algorithms from targets to canonical Gamma-prior results | `DPprior_a1()`, `DPprior_a2_newton()`, `DPprior_dual_hard()`, `DPprior_dual_soft()` |
