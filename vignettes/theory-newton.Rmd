...
if (!is.null(a2_fit$computation$trace)) {
  trajectory <- a2_fit$computation$trace[, c("a", "b")]
...
*Let $(\mu_K, \sigma^2_K)$ be feasible targets satisfying $1 < \mu_K < J$,
$0 < \sigma^2_K \leq (\mu_K - 1)(J - \mu_K)$, and suppose there exists $(a^*, b^*)$ with
...
cat(sprintf("Iterations:   %d\n", fit$computation$termination$iterations))
cat(sprintf("Termination:  %s (%s)\n",
            fit$computation$termination$code,
            fit$computation$termination$source))
print(fit[c("status", "usable", "verified", "message")])
final_residual <- sqrt(
  fit$residuals$K$mean^2 + fit$residuals$K$variance^2
)
...
trace_display <- fit$computation$trace[
  , c("iter", "a", "b", "M1", "V", "residual")
]
...
if (!is.null(fit$computation$trace) && nrow(fit$computation$trace) > 2) {
  trace_plot <- fit$computation$trace[
    fit$computation$trace$residual > 0,
  ]
...
for (tc in test_cases) {
  ...
  cat(sprintf("%-25s %6d %6.0f %6.0f %10d %10.2e\n",
              ...
              fit$computation$termination$iterations, fit_residual))
}
```
...
- $0 < \sigma^2_K \leq (\mu_K - 1)(J - \mu_K)$ (fixed-mean bounded-support variance bound)
...
a1_mom <- exact_K_moments(J, a1$parameters$a, a1$parameters$b)
...
cat(sprintf("  A1: Gamma(a = %.6f, b = %.6f)\n",
            a1$parameters$a, a1$parameters$b))
cat(sprintf("  A2: Gamma(a = %.6f, b = %.6f)\n",
            a2$parameters$a, a2$parameters$b))

cat("\nMoment Matching:\n")
cat(sprintf("  %-20s %12s %12s %12s\n", "", "Target", "A1", "A2"))
cat(sprintf("  %-20s %12.4f %12.4f %12.10f\n", "E[K]", mu_K,
            a1_mom$mean, a2$achieved$K$mean))
cat(sprintf("  %-20s %12.4f %12.4f %12.10f\n", "Var(K)", var_K,
            a1_mom$var, a2$achieved$K$variance))

cat("\nResidual ||F||:\n")
a1_residual <- sqrt((a1_mom$mean - mu_K)^2 + (a1_mom$var - var_K)^2)
cat(sprintf("  A1: %.6f\n", a1_residual))
a2_residual <- sqrt(a2$residuals$K$mean^2 + a2$residuals$K$variance^2)
cat(sprintf("  A2: %.2e\n", a2_residual))
cat(sprintf("  Improvement: %.0fx more accurate\n", a1_residual / a2_residual))
a2[c("status", "usable", "verified", "message")]
```
...
# Compute PMFs
pmf_a1 <- pmf_K_marginal(
  J, a1$parameters$a, a1$parameters$b, logS = logS
)
pmf_a2 <- pmf_K_marginal(
  J, a2$parameters$a, a2$parameters$b, logS = logS
)
...
a1_mom <- exact_K_moments(J, a1$parameters$a, a1$parameters$b)
a2_mom <- exact_K_moments(J, a2$parameters$a, a2$parameters$b)
...
cat("\nNote: In this benchmark, A2 is slower than A1 but much more accurate.\n")
...
5. **Error reduction:** A2 can reduce moment-matching residuals by orders of
   magnitude compared to A1 in the benchmark scenarios above
...
For new work, first construct `target_K <- DPprior_target_K(...)` and then
use A2 through `DPprior_fit(J, target_K = target_K, method = "A2-MN")` when
you need:
...
- **[A1 Approximation Theory](theory-approximations.html)**: Detailed analysis
  of A1 approximation errors and when they matter
