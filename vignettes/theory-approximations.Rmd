...
cat(sprintf("  CV(α) = 1/√a = %.4f\n", 1/sqrt(fit$parameters$a)))
fit[c("status", "usable", "verified", "message")]
```


### 4.4 Feasibility Region

**Novel Contribution.** The package's positive-variance moment workflows first
require the finite-support domain
$$
1 < \mu_K < J \quad \text{and} \quad 0 < \sigma^2_K \leq (\mu_K - 1)(J - \mu_K).
$$

Within that domain, the additional lower-bound feasibility constraint for A1 is:
...
This is a **proxy-model** constraint arising from the NegBin moment identity,
...
`DPprior_a1()` projects to the A1 lower-bound feasibility boundary. Boundary
means such as $\mu_K = J$ and fixed-mean variances above
$(\mu_K - 1)(J - \mu_K)$ are outside the package's positive-variance moment
workflow and are rejected rather than projected.
...
results <- do.call(rbind, lapply(1:nrow(test_cases), function(i) {
  ...
  exact <- exact_K_moments(J, fit$parameters$a, fit$parameters$b)
...
exact_a1 <- exact_K_moments(
  J, fit_a1$parameters$a, fit_a1$parameters$b
)

# A2 refinement through a reusable canonical target
target_K <- DPprior_target_K(J = J, mu_K = mu_K, var_K = var_K)
fit_a2 <- DPprior_fit(J = J, target_K = target_K, method = "A2-MN")
exact_a2 <- exact_K_moments(
  J, fit_a2$parameters$a, fit_a2$parameters$b
)
...
cat(sprintf("  Parameters: a = %.4f, b = %.4f\n",
            fit_a1$parameters$a, fit_a1$parameters$b))
...
cat(sprintf("  Parameters: a = %.4f, b = %.4f\n",
            fit_a2$parameters$a, fit_a2$parameters$b))
