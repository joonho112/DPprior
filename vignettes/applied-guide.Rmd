...
The primary v2 workflow constructs a reviewable target with
`DPprior_target_K()`, passes that object unchanged to `DPprior_fit()`, and then
uses `DPprior_dual_hard()` or `DPprior_dual_soft()` only when a second weight
statement is needed.
...
targets <- list(
  "High Uncertainty (var_K = 70)" =
    DPprior_target_K(J, mu_K = mu_K, var_K = 70),
  "Medium Uncertainty (var_K = 35)" =
    DPprior_target_K(J, mu_K = mu_K, var_K = 35),
  "Low Uncertainty (var_K = 21)" =
    DPprior_target_K(J, mu_K = mu_K, var_K = 21)
)
fits <- lapply(targets, function(target_K) {
  DPprior_fit(J, target_K = target_K, check_diagnostics = FALSE)
})
...
alpha_df <- do.call(rbind, lapply(names(fits), function(nm) {
  ...
  data.frame(
    alpha = alpha_grid,
    density = dgamma(alpha_grid, shape = fit$parameters$a,
...
k_df <- do.call(rbind, lapply(names(fits), function(nm) {
  fit <- fits[[nm]]
  pmf <- pmf_K_marginal(
    J, fit$parameters$a, fit$parameters$b, logS = logS
  )
  data.frame(
    K = 1:length(pmf),
    probability = pmf,
...
for (i in seq_along(mu_K_values)) {
  target_K <- DPprior_target_K(
    J, mu_K = mu_K_values[i], confidence = "low"
  )
  fit <- DPprior_fit(
    J, target_K = target_K, M = 160L, check_diagnostics = FALSE
  )
  comparison_df$a[i] <- fit$parameters$a
  comparison_df$b[i] <- fit$parameters$b
  comparison_df$E_alpha[i] <- fit$parameters$a / fit$parameters$b
  comparison_df$SD_alpha[i] <- sqrt(fit$parameters$a) / fit$parameters$b
}
...
```{r confidence-levels}
# Construct and review one canonical target per confidence level
target_low <- DPprior_target_K(
  J = 100, mu_K = 15, confidence = "low"
)
target_medium <- DPprior_target_K(
  J = 100, mu_K = 15, confidence = "medium"
)
target_high <- DPprior_target_K(
  J = 100, mu_K = 15, confidence = "high"
)
fit_low    <- DPprior_fit(
  J = 100, target_K = target_low, M = 160L, check_diagnostics = FALSE
)
fit_medium <- DPprior_fit(
  J = 100, target_K = target_medium, check_diagnostics = FALSE
)
fit_high <- DPprior_fit(
  J = 100, target_K = target_high, check_diagnostics = FALSE
)
...
confidence_comparison <- data.frame(
  ...
  var_K = round(c(fit_low$target$K$implied$variance,
                  fit_medium$target$K$implied$variance,
                  fit_high$target$K$implied$variance), 2),
  a = round(c(fit_low$parameters$a, fit_medium$parameters$a,
              fit_high$parameters$a), 3),
  b = round(c(fit_low$parameters$b, fit_medium$parameters$b,
              fit_high$parameters$b), 3),
  CV_alpha = round(1/sqrt(c(fit_low$parameters$a,
                            fit_medium$parameters$a,
                            fit_high$parameters$a)), 3)
)
...
target_direct <- DPprior_target_K(J = 100, mu_K = 15, var_K = 50)
fit_direct <- DPprior_fit(
  J = 100, target_K = target_direct, check_diagnostics = FALSE
)
cat("Direct specification: var_K = 50\n")
cat("  Gamma(a =", round(fit_direct$parameters$a, 3), ", b =",
    round(fit_direct$parameters$b, 3), ")\n")
cat("  Achieved E[K] =", round(fit_direct$achieved$K$mean, 4), "\n")
cat("  Achieved Var(K) =", round(fit_direct$achieved$K$variance, 4), "\n")
fit_direct[c("status", "usable", "verified", "message")]
```
...
target_quantile <- DPprior_target_K(
  J = 100, mu_K = mu_K_quantile, var_K = var_K_quantile
)
fit_quantile <- DPprior_fit(
  J = 100, target_K = target_quantile, check_diagnostics = FALSE
)
cat("\nElicited prior: Gamma(", round(fit_quantile$parameters$a, 3), ", ",
    round(fit_quantile$parameters$b, 3), ")\n", sep = "")
```
...
target_vif <- DPprior_target_K(J = 100, mu_K = 15, var_K = var_K_vif)
fit_vif <- DPprior_fit(
  J = 100, target_K = target_vif, check_diagnostics = FALSE
)
print(fit_vif)
```
...
```{r method-a1}
method_target_K <- DPprior_target_K(J = 100, mu_K = 15, var_K = 70)
fit_a1 <- DPprior_fit(
  J = 100, target_K = method_target_K, method = "A1",
  check_diagnostics = FALSE
)

cat("Method A1 (Closed-Form Approximation)\n")
cat("  Gamma(a =", round(fit_a1$parameters$a, 4), ", b =",
    round(fit_a1$parameters$b, 4), ")\n")
cat("  Target:   E[K] =", fit_a1$target$K$implied$mean, ", Var(K) =",
    fit_a1$target$K$implied$variance, "\n")
cat("  Achieved: E[K] =", round(fit_a1$achieved$K$mean, 4),
    ", Var(K) =", round(fit_a1$achieved$K$variance, 4), "\n")
print(fit_a1$residuals$K)
fit_a1[c("status", "usable", "verified", "message")]
```
...
+ Targets exact moment matching and reaches machine precision for feasible,
  well-conditioned targets

+ Often single-digit convergence; challenging targets may take more iterations
...
```{r method-a2-mn}
fit_a2 <- DPprior_fit(
  J = 100, target_K = method_target_K, method = "A2-MN",
  check_diagnostics = FALSE
)

cat("Method A2-MN (Exact Moment Matching)\n")
cat("  Gamma(a =", round(fit_a2$parameters$a, 4), ", b =",
    round(fit_a2$parameters$b, 4), ")\n")
cat("  Target:   E[K] =", fit_a2$target$K$implied$mean, ", Var(K) =",
    fit_a2$target$K$implied$variance, "\n")
cat("  Achieved: E[K] =", round(fit_a2$achieved$K$mean, 6),
    ", Var(K) =", round(fit_a2$achieved$K$variance, 6), "\n")
print(fit_a2$residuals$K)
cat("  Iterations:", fit_a2$computation$termination$iterations, "\n")
fit_a2[c("status", "usable", "verified", "message")]
```


### Method A2-KL: KL Divergence Minimization

The A2-KL method minimizes the Kullback-Leibler divergence between a target
distribution and the induced distribution of $K$. It can use a discretized
moment-based target or a custom PMF when you have a specific distributional
shape in mind.
...
+ More natural when a target PMF or distributional shape is available

+ Computationally more intensive than moment matching

```{r method-a2-kl}
fit_kl_condition <- NULL
fit_kl <- tryCatch(
  DPprior_fit(
    J = 100, target_K = method_target_K, method = "A2-KL",
    check_diagnostics = FALSE
  ),
  error = function(condition) {
    if (is.null(condition$result)) stop(condition)
    fit_kl_condition <<- condition
    condition$result
  }
)

cat("Method A2-KL (KL Divergence Minimization)\n")
cat("  Gamma(a =", round(fit_kl$parameters$a, 4), ", b =",
    round(fit_kl$parameters$b, 4), ")\n")
cat("  Target:   E[K] =", fit_kl$target$K$implied$mean, ", Var(K) =",
    fit_kl$target$K$implied$variance, "\n")
cat("  Achieved: E[K] =", round(fit_kl$achieved$K$mean, 4),
    ", Var(K) =", round(fit_kl$achieved$K$variance, 4), "\n")
cat("  KL residual:",
    format(fit_kl$residuals$distribution$kl, scientific = TRUE), "\n")
fit_kl[c("status", "usable", "verified", "message")]
if (!is.null(fit_kl_condition)) {
  cat("  Retained typed condition:", class(fit_kl_condition)[1], "\n")
}
```

The moment-derived KL target in this example can terminate as a retained
approximation. The typed condition preserves its canonical evidence for
review, but `usable = FALSE` means it must not be used as a calibrated prior.
For routine moment matching, use the verified A2-MN result; use A2-KL when the
distributional target and its method contract are substantively justified.
...
compare_methods <- function(J, mu_K, var_K) {
  target_K <- DPprior_target_K(J, mu_K = mu_K, var_K = var_K)
  retain_evidence <- function(method) tryCatch(
    DPprior_fit(
      J, target_K = target_K, method = method,
      check_diagnostics = FALSE
    ),
    error = function(condition) {
      if (is.null(condition$result)) stop(condition)
  ...
  a1 <- retain_evidence("A1")
  a2 <- retain_evidence("A2-MN")
  kl <- retain_evidence("A2-KL")
  
  data.frame(
    Method = c("A1", "A2-MN", "A2-KL"),
    a = round(c(a1$parameters$a, a2$parameters$a, kl$parameters$a), 4),
    b = round(c(a1$parameters$b, a2$parameters$b, kl$parameters$b), 4),
    `E[K]` = round(c(a1$achieved$K$mean, a2$achieved$K$mean,
                     kl$achieved$K$mean), 4),
    `Var(K)` = round(c(a1$achieved$K$variance,
                       a2$achieved$K$variance,
                       kl$achieved$K$variance), 4),
    Status = c(a1$status, a2$status, kl$status),
    Usable = c(a1$usable, a2$usable, kl$usable),
    Verified = c(a1$verified, a2$verified, kl$verified),
    check.names = FALSE
  )
}
...
For the package's positive-variance moment workflows, target moments must use
$1 < \mu_K < J$. The variance of $K$ is then bounded in two different senses:

+ **A1 lower bound**: $\text{Var}(K) > \mu_K - 1$ (approximately). This is
  a Negative Binomial proxy constraint used by A1, not a universal exact-DP
  constraint.

+ **Finite-support upper bound**: $\text{Var}(K) \leq (\mu_K - 1)(J - \mu_K)$. This is the
  sharp variance bound for a variable supported on $\{1,\ldots,J\}$ with fixed
  mean $\mu_K$.
...
target_infeasible <- DPprior_target_K(J = 100, mu_K = 15, var_K = 10)
fit_infeasible <- DPprior_fit(
  J = 100, target_K = target_infeasible, method = "A1",
  a1_projection = "nearest", check_diagnostics = FALSE
)

cat("  Requested var_K:", fit_infeasible$target$K$request$var_K, "\n")
cat("  Used var_K:", fit_infeasible$target$K$implied$variance, "\n")
cat("  (Projected to feasible boundary)\n")
```

The A1 method projects a specification that violates its proxy lower bound only
after explicit `a1_projection = "nearest"` opt-in; the default policy is an
error. The A2-MN method attempts exact matching without that Negative Binomial
lower-bound projection, but it still requires the package-wide moment domain
and can fail for truly infeasible or numerically ill-conditioned targets.
...
# =============================================================================
# Step 4: Construct and review the canonical target, then fit the prior
# =============================================================================
target_K <- DPprior_target_K(
  J = J,
  mu_K = mu_K,
...
cat("For Stan/JAGS:\n")
cat("  alpha ~ gamma(", round(fit$parameters$a, 4), ", ",
    round(fit$parameters$b, 4), ");\n\n", sep = "")

# Sample from the prior
n_samples <- 10000
alpha_samples <- rgamma(
  n_samples, shape = fit$parameters$a, rate = fit$parameters$b
)
...
sensitivity_mu <- lapply(mu_K_grid, function(mu) {
  target_K <- DPprior_target_K(J = J, mu_K = mu, confidence = "low")
  DPprior_fit(
    J = J, target_K = target_K, M = 160L,
    check_diagnostics = FALSE
  )
})
...
sensitivity_mu_df <- data.frame(
  `μ_K` = mu_K_grid,
  a = sapply(sensitivity_mu, function(x) round(x$parameters$a, 3)),
  b = sapply(sensitivity_mu, function(x) round(x$parameters$b, 3)),
  `E[α]` = sapply(sensitivity_mu, function(x) {
    round(x$parameters$a / x$parameters$b, 3)
  }),
  `P(W_SB > 0.5)` = sapply(sensitivity_mu, function(x) {
    round(prob_wsb_exceeds(0.5, x$parameters$a, x$parameters$b), 3)
  }),
  check.names = FALSE
)
...
sensitivity_conf <- lapply(confidence_levels, function(conf) {
  target_K <- DPprior_target_K(J = 100, mu_K = 15, confidence = conf)
  DPprior_fit(
    J = 100, target_K = target_K,
    M = if (identical(conf, "low")) 160L else 80L,
    check_diagnostics = FALSE
  )
})
...
sensitivity_conf_df <- data.frame(
  Confidence = c("Low", "Medium", "High"),
  var_K = sapply(sensitivity_conf, function(x) {
    round(x$target$K$implied$variance, 2)
  }),
  a = sapply(sensitivity_conf, function(x) round(x$parameters$a, 3)),
  b = sapply(sensitivity_conf, function(x) round(x$parameters$b, 3)),
  `CV(α)` = sapply(sensitivity_conf, function(x) {
    round(1/sqrt(x$parameters$a), 3)
  }),
  `Median W_SB` = sapply(sensitivity_conf, function(x) {
    round(quantile_w1(0.5, x$parameters$a, x$parameters$b), 3)
  }),
  check.names = FALSE
)
...
for (i in 1:nrow(grid_results)) {
  target_K <- DPprior_target_K(
    J = 100, mu_K = grid_results$mu_K[i],
    confidence = grid_results$confidence[i]
  )
  fit <- DPprior_fit(
    J = 100, target_K = target_K,
    M = if (identical(
      grid_results$confidence[i], "low"
    )) 160L else 80L,
    check_diagnostics = FALSE
  )
  grid_results$a[i] <- round(fit$parameters$a, 3)
  grid_results$b[i] <- round(fit$parameters$b, 3)
  grid_results$E_alpha[i] <- round(fit$parameters$a / fit$parameters$b, 3)
}
...
| 5 | Fit prior | `fit <- DPprior_fit(J, target_K = target_K, ...)` |
| 6 | Visualize | `plot(fit)` |
| 7 | Sensitivity analysis | Repeat with different specifications |
| 8 | Use in model | `alpha ~ gamma(fit$parameters$a, fit$parameters$b)` |
