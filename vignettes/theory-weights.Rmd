...
Despite this caveat, $w_1=W_{SB}$ remains a meaningful named weight diagnostic.
If $P(W_{SB}>0.5)$ is high, then a randomly selected unit is likely to belong
to a cluster that contains more than half of the population. This is not a
claim about $P(W_{max}>0.5)$.
...
**First-size-biased-weight survival function:**
$$
...
w1_df <- data.frame(
  ...
  survival = prob_wsb_exceeds(x_grid, a, b)
)
...
correspond to a large $W_{SB}$ tail probability.

```{r unintended-prior-demo, fig.cap="The K-calibrated prior (blue) achieves the target E[K] but implies a larger W_SB tail probability than practitioners might expect."}
# Demonstrate the unintended prior problem
J <- 50
mu_K_target <- 5

# Construct and fit the canonical K target
target_K <- DPprior_target_K(J = J, mu_K = mu_K_target, var_K = 8)
fit_K <- DPprior_fit(
...
cat(sprintf("  Gamma hyperprior: a = %.3f, b = %.3f\n",
            fit_K$parameters$a, fit_K$parameters$b))
cat(sprintf("  Achieved E[K] = %.2f\n", fit_K$achieved$K$mean))
cat("\nImplied weight behavior:\n")
cat(sprintf("  E[w₁] = %.3f\n",
            mean_w1(fit_K$parameters$a, fit_K$parameters$b)))
cat(sprintf("  P(w₁ > 0.5) = %.3f\n",
            prob_wsb_exceeds(0.5, fit_K$parameters$a, fit_K$parameters$b)))
cat(sprintf("  P(w₁ > 0.9) = %.3f\n",
            prob_wsb_exceeds(0.9, fit_K$parameters$a, fit_K$parameters$b)))
...
implied_df <- data.frame(
  x = x_grid,
  density = density_w1_manual(
    x_grid, fit_K$parameters$a, fit_K$parameters$b
  ),
...
ggplot(implied_df, aes(x = x, y = density)) +
  ...
  labs(
    ...
    subtitle = sprintf("J = %d, target E[K] = %d; P(W_SB > 0.5) = %.2f",
                       J, mu_K_target,
                       prob_wsb_exceeds(
...
**Definition 3** (Soft dual-anchor optimization; this work).
...
This is the soft contract. The v2 hard contract instead minimizes a fixed-scale
K loss subject to a named inequality $g_w(a,b)\leq0$ and has no $\lambda$.
...
The formulas below illustrate possible scientific discrepancies. The v2 public
soft API uses fixed input-derived scales and records them in `tradeoff$scales`
and `computation$scaling`; the hard API independently verifies its inequality.
...
**Probability constraint for $W_{SB}$:**
Target a specific first-size-biased-weight tail probability:
$$
\mathcal{L}(a, b) = \lambda \cdot L_K(a, b) + 
...
```{r dual-anchor-demo}
# Demonstrate a verified hard dual-anchor calibration
...
cat("Hard dual-anchor calibration (bound: P(w₁ > 0.5) <= 0.25):\n")
cat(sprintf("  Gamma hyperprior: a = %.3f, b = %.3f\n",
            fit_dual$parameters$a, fit_dual$parameters$b))
cat(sprintf("  Achieved E[K] = %.2f (target: %.1f)\n", 
            fit_dual$achieved$K$mean, mu_K_target))
cat(sprintf("  P(w₁ > 0.5) = %.3f (bound: 0.25)\n",
            fit_dual$achieved$weight$value))
...
tradeoff_results <- lapply(lambda_grid, function(lam) {
  ...
  list(
    ...
    P_WSB_gt_50 = prob_wsb_exceeds(
      0.5, fit$parameters$a, fit$parameters$b
    ),
    E_WSB = mean_w1(fit$parameters$a, fit$parameters$b),
    status = fit$status,
    usable = fit$usable,
...
tradeoff_df <- data.frame(
  ...
  P_WSB = sapply(tradeoff_results, `[[`, "P_WSB_gt_50"),
  E_WSB = sapply(tradeoff_results, `[[`, "E_WSB"),
  status = sapply(tradeoff_results, `[[`, "status"),
  usable = sapply(tradeoff_results, `[[`, "usable"),
  verified = sapply(tradeoff_results, `[[`, "verified"),
  message = sapply(tradeoff_results, `[[`, "message")
)

knitr::kable(
  tradeoff_df[, c("lambda", "status", "usable", "verified", "message")],
  caption = "Status review for every soft trade-off candidate"
)

ggplot(tradeoff_df, aes(x = E_K, y = P_WSB)) +
  ...
  annotate("text", x = 4.5, y = 0.22, label = "W_SB target",
...
`allow_approximate = TRUE` retains any non-converged candidate only so its
status and residual evidence can be reviewed in the sensitivity table. A row
with `usable = FALSE` is not an approved calibrated prior.
...
**Consequence:** At the level of this mean identity, a numeric equation for
$w_1$ and one for $\rho$ coincide. Their estimands do not. The v2 public
hard/soft target grammar supports named $W_{SB}$ metrics and does not silently
relabel them as direct $\rho$ claims; use $\rho$ as a separate diagnostic.
...
**Recommendation:** Use a named $W_{SB}$ metric for v2 hard/soft calibration.
Use $\rho$ when the co-clustering interpretation resonates with domain experts,
but report it as a diagnostic unless a future API explicitly supports that
estimand.
...
| `prob_wsb_exceeds(t, a, b)` | Explicitly named closed-form $P(W_{SB} > t)$ |
...
| `DPprior_target_K()` | Canonical bounded-discrete K target |
| `DPprior_fit()` | K-only calibration from the canonical target |
| `DPprior_dual_hard()` | Verified named weight inequality |
| `DPprior_dual_soft()` | Fixed-scale K/weight trade-off |
...
for (t in c(0.3, 0.5, 0.9)) {
  ...
  analytic_prob <- prob_wsb_exceeds(t, a, b)
...
and is not a hard-constraint certificate. Removal is not permitted before
v3.0 and a migration review; new work should use `DPprior_dual_hard()` or
`DPprior_dual_soft()`.
...
5. **K-only calibration** can induce unintended weight behavior. The
   **dual-anchor framework** provides a verified hard-inequality contract and
   a separate fixed-scale soft trade-off contract; only soft mode uses
   $\lambda$.
