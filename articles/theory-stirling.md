...
machinery underlying the Antoniak distribution—the exact probability
mass function (PMF) for the number of distinct clusters $K_{J}$ in a
Dirichlet Process (DP) model. We focus specifically on **unsigned
Stirling numbers of the first kind** and their numerically stable
computation, which forms the core of the DPprior package’s reference
computational layer.

This vignette is intended for statisticians and methodological
researchers who want to understand the exact implementation details. We
cover:

1.  The combinatorial definition and properties of unsigned Stirling
    numbers
...
combinatorics and Bayesian nonparametrics literature and
**implementation contributions** of this package.

## 1. Unsigned Stirling Numbers of the First Kind

### 1.1 Definition and Combinatorial Interpretation

The **unsigned Stirling numbers of the first kind**, denoted
$\left| s(J,k) \right|$, count the number of permutations of $J$
elements that consist of exactly $k$ disjoint cycles.

**Example.** For $J = 3$ elements $\{ 1,2,3\}$, the permutations grouped
by number of cycles are:

| $k$ | Permutations                    | Count $\parallel s(3,k) \parallel$ |
|-----|---------------------------------|------------------------------------|
| 1   | $(1\ 2\ 3),(1\ 3\ 2)$           | 2                                  |
| 2   | $(1)(2\ 3),(2)(1\ 3),(3)(1\ 2)$ | 3                                  |
| 3   | $(1)(2)(3)$                     | 1                                  |

The total is $2 + 3 + 1 = 6 = 3!$, as expected since we’re partitioning
all permutations.

**Attribution.** Stirling numbers have a rich history in combinatorics,
dating back to James Stirling’s 1730 work *Methodus Differentialis*. The
connection to the DP cluster distribution was established by Antoniak
(1974).
...
**Combinatorial interpretation:** When adding element $J$ to a
permutation of elements $\{ 1,\ldots,J - 1\}$:

- Element $J$ can form a **new cycle by itself**: this accounts for
  $\left| s(J - 1,k - 1) \right|$ permutations (we need $k - 1$ cycles
  in the first $J - 1$ elements)
- Element $J$ can be **inserted into any position** within an existing
  cycle: there are $(J - 1)$ positions to insert, and we need $k$ cycles
  already present, giving $(J - 1) \cdot \left| s(J - 1,k) \right|$
...
$$\begin{aligned}
\left| s(0,0) \right| & {= 1\quad\text{(empty permutation: one way to arrange nothing)}} \\
\left| s(J,0) \right| & {= 0\quad{\text{for}\mspace{6mu}}J \geq 1{\mspace{6mu}\text{(no permutation has 0 cycles)}}} \\
\left| s(J,J) \right| & {= 1\quad{\text{for all}\mspace{6mu}}J{\mspace{6mu}\text{(only the identity has}\mspace{6mu}}J{\mspace{6mu}\text{cycles)}}}
\end{aligned}$$
...
$$\sum\limits_{k = 1}^{J}\left| s(J,k) \right| = J!$$
...
**The key insight:** Direct computation in the original scale is
impossible for any practical $J$. We must work entirely in log-space.

### 2.2 Why Naive Log-Space Fails

One might think: “Just take logs of everything!” But consider the
recurrence:
...
$$\log\left| s(J,k) \right| = \log\left( e^{L_{J - 1,k - 1}} + (J - 1) \cdot e^{L_{J - 1,k}} \right)$$

where $L_{J,k}:=\log\left| s(J,k) \right|$. The problem: we need to
compute $\log\left( e^{a} + e^{b} \right)$ where $a$ and $b$ may be very
large (causing overflow when exponentiating) or very different in
magnitude (causing underflow when adding).

## 3. The `logsumexp` Trick

### 3.1 Mathematical Foundation

The `logsumexp` function computes $\log\left( e^{a} + e^{b} \right)$ in
a numerically stable way using the identity:
...
3.  We use `log1p(x)` = $\log(1 + x)$ for numerical stability near
    $x = 0$
...
For computing $\log\sum_{i = 1}^{n}e^{x_{i}}$, we use the vectorized
extension:

$$\log\sum\limits_{i = 1}^{n}e^{x_{i}} = \max\limits_{i}\left( x_{i} \right) + \log\sum\limits_{i = 1}^{n}e^{x_{i} - \max\limits_{i}{(x_{i})}}$$
...
function builds the complete lower-triangular matrix of log-Stirling
numbers using the recursion:

$$L_{J,k} = \text{logsumexp}\left( L_{J - 1,k - 1},\log(J - 1) + L_{J - 1,k} \right)$$

**Complexity:** $O\left( J_{\max}^{2} \right)$ time and space.
...
We can verify the computation by checking that row sums equal
$\log(J!)$:
...
$$\Pr(K_{J} = k\left| \alpha) = \right|s(J,k)| \cdot \frac{\alpha^{k}}{(\alpha)_{J}},\quad k = 1,\ldots,J$$

where
$(\alpha)_{J} = \alpha(\alpha + 1)\cdots(\alpha + J - 1) = \frac{\Gamma(\alpha + J)}{\Gamma(\alpha)}$
...
in modern DP prior-elicitation work including Dorazio (2009), Murugiah
and Sweeting (2012), and Vicentini and Jermyn (2025). Zito, Rigon, and
Dunson (2024) present the general Gibbs-type form
$\Pr\left( K_{J} = k \right) = V_{J,k}\left| s(J,k) \right|$.

### 5.2 Log-Space Computation

In log-space, the PMF becomes:

$$\log\Pr(K_{J} = k\left| \alpha) = \log \right|s(J,k)| + k\log\alpha - \log(\alpha)_{J}$$

where $\log(\alpha)_{J} = \log\Gamma(\alpha + J) - \log\Gamma(\alpha)$.
...
observations.](theory-stirling_files/figure-html/fig-antoniak-pmf-1.png)
...
$$K_{J}\overset{d}{=}\sum\limits_{i = 1}^{J}B_{i},\quad B_{i} \sim \text{Bernoulli}\left( \frac{\alpha}{\alpha + i - 1} \right)$$

This provides an independent verification of our Stirling-based
computation:
...
Poisson-Binomial
representation.](theory-stirling_files/figure-html/verify-poisson-binomial-1.png)
...
concentrated at $K_{J} = 1$, with
$\left. \Pr\left( K_{J} = 1|\alpha \right)\rightarrow 1 \right.$ as
$\left. \alpha\rightarrow 0 \right.$. This requires careful handling of
probabilities very close to 1.

**Very large $\alpha$ (\> 50):** The distribution shifts toward
$K_{J} \approx J$, and the log-probabilities for small $k$ become
extremely negative.
...
2.  **Stable softmax:** Uses the shifted exponential trick to prevent
    overflow:
    $p_{i} = \exp\left( x_{i} - \max_{j}x_{j} \right)/\sum_{j}\exp\left( x_{j} - \max_{j}x_{j} \right)$

3.  **Input validation:** The
    [`assert_positive()`](https://joonho112.github.io/DPprior/reference/assert_positive.md)
    and
    [`assert_valid_J()`](https://joonho112.github.io/DPprior/reference/assert_valid_J.md)
...
The conditional moments of $K_{J}|\alpha$ involve the digamma function:

$$\mu_{J}(\alpha) = \alpha \cdot \left\lbrack \psi(\alpha + J) - \psi(\alpha) \right\rbrack$$

For very small $\alpha$,
$\left. \psi(\alpha)\rightarrow - \infty \right.$, which can cause
...
Understanding how the distribution of $K_{J}$ changes with sample size
$J$ is crucial for prior elicitation. For fixed $\alpha$:
...
![Effect of sample size J on the distribution of K for fixed α =
2.](theory-stirling_files/figure-html/fig-J-effect-1.png)
...
1.  **Unsigned Stirling numbers** $\left| s(J,k) \right|$ count
    permutations with exactly $k$ cycles and satisfy a well-defined
    recurrence relation.
...
3.  **The `logsumexp` trick** enables stable computation of
    $\log\left( e^{a} + e^{b} \right)$ for any values of $a$ and $b$.

4.  **The Antoniak distribution** provides the exact PMF of
    $K_{J}|\alpha$ via Stirling numbers, computed stably in log-space.

5.  **DPprior implementation:** Pre-computes and caches the log-Stirling
    matrix, enabling fast PMF evaluation for any $\alpha$.

The stable computation of the Antoniak PMF forms the foundation for the
package’s exact moment-matching algorithms (A2) described in subsequent
vignettes.
...
| `verify_stirling_row_sum(logS)`     | Verify $\sum_{k} \parallel s(J,k) \parallel = J!$ identity |
| `logsumexp(a, b)`                   | Numerically stable $\log\left( e^{a} + e^{b} \right)$      |
| `logsumexp_vec(x)`                  | Vectorized log-sum-exp                                     |
| `softmax(x)`                        | Numerically stable softmax                                 |
| `pmf_K_given_alpha(J, α, logS)`     | Antoniak PMF for $K_{J} \parallel \alpha$                  |
