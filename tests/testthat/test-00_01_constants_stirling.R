...
test_that("compute_log_stirling input validation works", {
  expect_error(
    compute_log_stirling(-1), "non-negative integer",
    class = "dpprior_bounds_error"
  )
  expect_error(
    compute_log_stirling(1000), "exceeds maximum",
    class = "dpprior_bounds_error"
  )
  expect_error(
    compute_log_stirling(1.5), "non-negative integer",
    class = "dpprior_integer_error"
  )
  expect_error(
    compute_log_stirling(Inf), "non-negative integer",
    class = "dpprior_nonfinite_error"
  )
  expect_error(compute_log_stirling(NaN), "non-negative integer")
  expect_error(
    compute_log_stirling("10"), "non-negative integer",
...
# =============================================================================
# Independent recurrence, large-J, and cache-contract tests
# =============================================================================

test_that("log arithmetic satisfies translation and monotonicity invariants", {
  x <- c(-1000, -20, 0, 15, 1000)
  shifts <- c(-500, -1, 0, 3.25, 500)

  for (shift in shifts) {
    expect_equal(
      logsumexp_vec(x + shift),
      logsumexp_vec(x) + shift,
      tolerance = 2e-13,
  ...
  expect_equal(logsumexp(x, rev(x)), logsumexp(rev(x), x))
  expect_true(all(logsumexp(x, rev(x)) >= pmax(x, rev(x))))
  expect_equal(
    logsumexp_vec(c(-Inf, log(2), log(3))),
    log(5),
    tolerance = 1e-14
  )
})
...
test_that("small-J Stirling table matches an independent integer recurrence", {
  J_max <- 12L
  reference <- matrix(0, nrow = J_max + 1L, ncol = J_max + 1L)
  reference[1L, 1L] <- 1

  # This oracle deliberately uses ordinary exact integer-valued arithmetic,
  # not DPprior log arithmetic or its recurrence implementation.
  for (J in seq_len(J_max)) {
    for (k in 0:J) {
      from_new_cycle <- if (k >= 1L) reference[J, k] else 0
      from_insert <- if (k <= J - 1L) (J - 1L) * reference[J, k + 1L] else 0
      reference[J + 1L, k + 1L] <- from_new_cycle + from_insert
    }
  }

  .clear_stirling_cache()
  actual <- compute_log_stirling(J_max)
  for (J in 0:J_max) {
    for (k in 0:J) {
      value <- reference[J + 1L, k + 1L]
      if (value == 0) {
        expect_equal(actual[J + 1L, k + 1L], -Inf)
      } else {
        expect_equal(
          exp(actual[J + 1L, k + 1L]),
          value,
          tolerance = 1e-10,
          info = sprintf("J=%d k=%d", J, k)
        )
...
test_that("large-J Stirling rows retain exact identities and normalization", {
  .clear_stirling_cache()
  logS <- compute_log_stirling(500L)
  row_500 <- logS[501L, 2L:501L]

  expect_true(all(is.finite(row_500)))
  expect_equal(row_500[1L], lgamma(500), tolerance = 1e-9)
  expect_equal(row_500[500L], 0, tolerance = 0)
  expect_equal(row_500[499L], log(choose(500, 2)), tolerance = 1e-11)
  expect_equal(logsumexp_vec(row_500), lgamma(501), tolerance = 1e-9)

  normalized <- exp(row_500 - lgamma(501))
  expect_true(all(normalized >= 0 & normalized <= 1))
  expect_equal(sum(normalized), 1, tolerance = 1e-11)

  # Row totals log(J!) are strictly increasing for J >= 2.
  selected_J <- c(2L, 10L, 50L, 100L, 250L, 500L)
  log_totals <- vapply(selected_J, function(J) {
    logsumexp_vec(logS[J + 1L, 2L:(J + 1L)])
  }, numeric(1))
  expect_true(all(diff(log_totals) > 0))
})


test_that("Stirling cache is bounded, reusable, and not user-mutable", {
  .clear_stirling_cache()
  full <- compute_log_stirling(500L)
  after_cold <- .stirling_cache_info()

  expect_identical(after_cold$policy, "single-largest-table")
  expect_identical(after_cold$entries, 1L)
  expect_identical(after_cold$max_J, 500L)
  expect_identical(after_cold$misses, 1L)
  expect_identical(after_cold$hits, 0L)
  expect_lte(after_cold$bytes, 8 * 501^2 + 4096)

  small <- compute_log_stirling(50L)
  expect_equal(dim(small), c(51L, 51L))
  after_hit <- .stirling_cache_info()
  expect_identical(after_hit$hits, 1L)
  expect_identical(after_hit$max_J, 500L)

  # R copy-on-modify must keep a caller mutation out of the retained table.
  small[2L, 2L] <- 99
  small_again <- compute_log_stirling(50L)
  expect_equal(small_again[2L, 2L], 0)
  expect_equal(full[501L, 501L], 0)
})

test_that("logical controls require ordinary dimensionless scalars", {
  malformed <- list(
    matrix(FALSE, nrow = 1L),
    array(TRUE, dim = c(1L, 1L, 1L)),
    structure(FALSE, class = "dpprior_test_logical")
  )
  for (value in malformed) {
    condition <- tryCatch(
      .dpprior_validate_control(value, "flag", type = "logical"),
      error = identity
    )
    expect_s3_class(condition, "dpprior_invalid_input")
    expect_s3_class(condition, "dpprior_type_error")
  }
})
