...
test_that("rho conditional moments avoid premature overflow", {
  alphas <- c(1e-300, 1e80, 1e100)
  log_var_reference <- log(2) + log(alphas) -
    2 * log1p(alphas) - log(alphas + 2) - log(alphas + 3)
  log_second_reference <- log(alphas + 6) -
    log(alphas + 1) - log(alphas + 2) - log(alphas + 3)

  expect_equal(
    var_rho_given_alpha(alphas),
    exp(log_var_reference),
    tolerance = 1e-13
  )
  expect_equal(
    DPprior:::mean_rho_sq_given_alpha(alphas),
    exp(log_second_reference),
    tolerance = 1e-13
  )
  expect_gt(var_rho_given_alpha(1e80), 0)
  expect_gt(var_rho_given_alpha(1e100), 0)
})
...
test_that("summary_rho returns correct structure", {
  ...
  expect_true(all(c(
    "status", "usable", "verified", "message", "estimand", "label",
    "units", "mean", "var", "sd", "cv", "params", "alpha_prior",
    "conditional_at_alpha_mean", "verification", "provenance"
  ) %in% names(result)))
  expect_identical(result$estimand, "rho")
  expect_identical(result$units, "probability")
  expect_identical(result$status, "converged")
  expect_true(result$verified)
  expect_null(result$interpretation)
})
...
test_that("summary_rho reports numeric estimand without uncalibrated category", {
  result_low_alpha <- summary_rho(a = 0.5, b = 1)
  result_high_alpha <- summary_rho(a = 10, b = 1)

  expect_null(result_low_alpha$interpretation)
  expect_null(result_high_alpha$interpretation)
  expect_gt(result_low_alpha$mean, result_high_alpha$mean)
  expect_identical(result_low_alpha$estimand, "rho")
  expect_identical(result_high_alpha$estimand, "rho")
  output <- capture.output(print(result_low_alpha))
  expect_true(any(grepl("Estimand:", output, fixed = TRUE)))
  expect_false(any(grepl("Interpretation|High|Moderate|Low", output)))
})
...
test_that("rrho generates correct number of samples", {
  ...
  diagnostics <- attr(samples, "rrho_diagnostics", exact = TRUE)
  expect_identical(diagnostics$status, "converged")
  expect_true(diagnostics$verified)
  expect_lte(diagnostics$max_remainder_bound,
             diagnostics$remainder_tolerance)
})

test_that("rrho adaptively controls high-alpha remainder and MC bias", {
  set.seed(20260819)
  samples <- rrho(
    500, a = 100, b = 0.1, n_sticks = 500,
    remainder_tol = 1e-7, max_sticks = 30000
  )
  diagnostics <- attr(samples, "rrho_diagnostics", exact = TRUE)

  # Independent adaptive integration over the central Gamma mass.
  lower <- stats::qgamma(1e-12, shape = 100, rate = 0.1)
  upper <- stats::qgamma(1 - 1e-12, shape = 100, rate = 0.1)
  reference <- stats::integrate(
    function(alpha) {
      exp(stats::dgamma(alpha, 100, rate = 0.1, log = TRUE) -
            log1p(alpha))
    },
    lower, upper, rel.tol = 1e-11
  )$value

  expect_identical(diagnostics$status, "converged")
  expect_gt(stats::median(diagnostics$sticks_used), 500)
  expect_lte(diagnostics$max_remainder_bound, 1e-7)
  expect_lt(abs(mean(samples) - reference), 3e-5)
})

test_that("rrho exposes approximate truncation or raises a typed failure", {
  set.seed(811)
  expect_error(
    rrho(
      1, a = 1e6, b = 1, n_sticks = 10, max_sticks = 10,
      remainder_tol = 1e-12
    ),
    class = "dpprior_rho_truncation_error"
  )

  set.seed(811)
  sample <- rrho(
    1, a = 1e6, b = 1, n_sticks = 10, max_sticks = 10,
    remainder_tol = 1e-12, strict = FALSE
  )
  diagnostics <- attr(sample, "rrho_diagnostics", exact = TRUE)
  expect_identical(diagnostics$status, "approximate")
  expect_false(diagnostics$usable)
  expect_gt(diagnostics$max_remainder_bound,
            diagnostics$remainder_tolerance)
})

test_that("rrho diagnostics keep subnormal remainder bounds conservative", {
  smallest_positive <- .Machine$double.xmin * .Machine$double.eps
  set.seed(1001)
  sample <- rrho(
    1, a = 100, b = 1, n_sticks = 10,
    remainder_tol = smallest_positive, max_sticks = 100000
  )
  diagnostics <- attr(sample, "rrho_diagnostics", exact = TRUE)

  expect_identical(diagnostics$status, "converged")
  expect_gt(diagnostics$remainder_bounds[[1L]], 0)
  expect_lte(
    diagnostics$log_remainder_bounds[[1L]],
    log(diagnostics$remainder_tolerance)
  )
  expect_gte(
    log(diagnostics$remainder_bounds[[1L]]),
    diagnostics$log_remainder_bounds[[1L]]
  )
})
...
test_that("rrho rejects invalid input", {
  expect_error(rrho(-1, a = 2, b = 1), class = "dpprior_bounds_error")
  expect_error(rrho(100, a = -1, b = 1), "must be finite and positive")
  expect_error(rrho(100, a = 2, b = 0), "must be finite and positive")
  expect_error(rrho(1, a = c(1, 2), b = 1),
               class = "dpprior_length_error")
  expect_error(rrho(1, a = 1, b = 1, remainder_tol = 0),
               class = "dpprior_bounds_error")
  expect_error(rrho(1, a = 1, b = 1, n_sticks = 20, max_sticks = 10),
               class = "dpprior_bounds_error")
})
